#!/usr/bin/env python3
"""
Generate Key Figures Script
This script generates 5 specific visualizations from the descriptive statistics analysis:
...
def generate_ai_roles_over_time_plot(usdf):
    ...
    pct_df_all.drop(pct_df_all.index[-1], inplace=True)  # Remove last incomplete quarter
    ...
    plt.savefig('../results/figures/pct_ai_roles_overall_industry.png', bbox_inches='tight')
    plt.show()
...
def generate_benefit_differences_plot(usdf):
    ...
    # Create plot
    fig, ax = plt.subplots(figsize=(10, 6))
    pct_diff_df.plot(ax=ax, color=list(benefit_colors.values())[:4] + [benefit_colors['CULTURE'], benefit_colors['REMOTE_KW']])
...
def generate_benefits_by_role_plot(usdf):
    ...
    plt.savefig('../results/figures/pct_jobs_by_benefit_and_role_type/benefits_ai_role_colors_10m_2024.png', bbox_inches='tight')
    plt.show()
...
def plot_benefits_over_time(merged_data, benefit, period):
    ...
    pivot_data = pivot_data[['Yes'] + [col for col in pivot_data.columns if col != 'Yes']]
...
def get_benefit_by_occupation(usdf_select, benefit):
    ...
    occ_ai_benefit_group = occ_ai_benefit_group.merge(occ_ai_group, on=[occupation, 'AI ROLE'], how='left')
