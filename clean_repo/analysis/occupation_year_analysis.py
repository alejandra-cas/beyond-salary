...
try:
    ...
    benefits4_labels = ['Tuition Assistance', 'Paid Leave', 'Health and Wellbeing', 'Parental Leave', 'Workplace Culture', 'Remote Keywords']
...
def load_occupation_year_data(occ_year_data_path):
    ...
    # Add log job count
    df['Log Job Count'] = df['job_count'].apply(lambda x: np.log(x + 1))  # Add 1 to avoid log(0)
...
def run_regression_models(df, include_ai_count=False):
    ...
    print(f"Running {model_type.lower()} balanced sample difference regression models...")
...
def generate_combined_tables(diff_models, diff_models_2, output_dir):
    ...
    html_table = create_regression_table_html(valid_models, valid_labels, html_output_path)
    ...
    latex_table = create_regression_table_latex(valid_models, valid_labels, latex_output_path)
