...
def plot_wage_vs_perk_airole_betas(wage_df, perk_df, output_dir=None):
    """
    For each perk, scatterplot AI wage beta (x) against AI perk beta (y)
    across industry-year cells.
    """
    ...
    merged = perk_df.merge(wage_df, on=["industry", "year"], suffixes=("_perk", "_wage"))
    ...
    plt.suptitle("AI Wage Premium vs. AI Perk Premium\n(by Industry-Year Cell)", fontsize=14, y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.95])

    out_path = output_dir / "wage_vs_perk_airole_betas_scatterplot.png"
...
def save_coefficient_table(wage_df, perk_df, output_dir=None):
    ...
    # Wage betas
    wage_out = output_dir / "industry_year_wage_airole_betas.csv"
    wage_df.to_csv(wage_out, index=False)
    print(f"Saved: {wage_out}")

    # Perk betas
    perk_out = output_dir / "industry_year_perk_airole_betas.csv"
    perk_df.to_csv(perk_out, index=False)
    print(f"Saved: {perk_out}")

    # Summary: correlation between wage and perk betas per benefit
    if wage_df.empty or perk_df.empty:
        print("No coefficients to summarize.")
        return
    merged = perk_df.merge(wage_df, on=["industry", "year"])
    summary_rows = []
    for benefit in benefits:
    ...
    summary_out = output_dir / "wage_perk_airole_beta_correlation_summary.csv"
...
def main():
    ...
    print("\n--- Step A: Estimating wage betas (OLS) ---")
    wage_df = estimate_wage_betas(data)
    print(f"Estimated {len(wage_df)} industry-year wage betas")

    print("\n--- Step B: Estimating perk betas (Logit) ---")
    perk_df = estimate_perk_betas(data)
    print(f"Estimated {len(perk_df)} industry-year-perk betas")

    print("\n--- Step C: Producing outputs ---")
    save_coefficient_table(wage_df, perk_df)
    plot_wage_vs_perk_airole_betas(wage_df, perk_df)
